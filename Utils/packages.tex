%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Packages
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{blindtext}
\usepackage{comment}
\usepackage{csquotes}
\usepackage{algorithm}
\usepackage[noend]{algpseudocode}
\usepackage{pifont}
\usepackage{tabularx}
\usepackage{rotating}
\usepackage{graphicx}
\usepackage{amsmath}        % Mathematische Umgebungen und Symbole
\usepackage{xcolor}         % Farbnamen und Definition
\usepackage{subcaption}     % Subcaption für Unterabbildungen
\usepackage{array}
\usepackage{geometry}
\usepackage{url}            % For proper formatting of URLs
\usepackage{ulem}           % For strikethrough text
\usepackage{booktabs}
\usepackage{amssymb}   % for \checkmark
\usepackage{pifont}    % optional, for ✗
\usepackage{multirow}
\usepackage{array}
\usepackage{tikz}
\usetikzlibrary{decorations.pathreplacing}

% Use either minted OR listings, not both to avoid lol conflicts
\usepackage{minted}
% Alternative: \usepackage{listings}

\usepackage{xurl} % bessere Umbruchstellen in \url
% \usepackage[hidelinks]{hyperref} % falls noch nicht drin

% Geometry settings
\geometry{a4paper, margin=1in}

% Only include listings settings if using listings package
% \lstset{
%   basicstyle=\ttfamily,
%   breaklines=true,     % Enables automatic line breaking
%   linewidth=\linewidth % Limits the width to the current line width
% }

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Citation Style

\usepackage[
    backend=biber,
    style=ieee,
    citestyle=numeric-comp,
    bibstyle=ieee,
    maxcitenames=2,
    mincitenames=1,
    maxbibnames=99,
    minbibnames=99
]{biblatex}

\addbibresource{Literature.bib}

\newcommand{\cmark}{\ding{51}} % ✓
\newcommand{\xmark}{\ding{55}} % ✗
\newcommand{\algcomment}[1]{\textcolor{green!35!black}{\textbf{// #1}}}
% chktex-file 44