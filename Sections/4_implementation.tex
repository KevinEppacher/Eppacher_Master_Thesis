...
\textbf{Experiment 1: Overall Performance Benchmarking}
...
\textbf{Experiment 2: Exploration--Memory Fusion Weighting}
...
\textbf{Experiment 3: Sensitivity to Semantic Map Granularity}
...
\textbf{Experiment 4: Robustness to False Positives Through Multi-Source Detection Fusion}
...
\begin{itemize}
    ...
    \item \textbf{Fusion Model:}  
    Each semantic source contributes a weighted score to the final fused confidence:
    \[
        S_{\text{fusion}} = w_d S_{\text{det}} \;+\; w_c S_{\text{cosine}} \;+\; w_m S_{\text{memory}},
    \]
    where \( w_d, w_c, w_m \) determine the influence of detection confidence, VLM similarity, and memory-based likelihood, respectively.

    \item \textbf{Research Questions:}
    \begin{itemize}
        \item \textbf{RQ3a:} How does overall task performance (SR) change under different weight configurations \( (w_d, w_c, w_m) \)?
        \item \textbf{RQ3b:} How do precision, recall, and F1-Score vary across:
        \begin{itemize}
            \item COCO categories (closed-set),
            \item open-vocabulary object classes,
            \item zero-shot categories absent from detector training?
        \end{itemize}
        \item \textbf{RQ3c:} Which drawbacks arise when increasing detection thresholds or overemphasising individual score sources (e.g., loss of low-confidence but valid detections)?
    \end{itemize}

    \item \textbf{Evaluation Metrics:}  
    Robustness is quantified using semantic classification metrics derived from the confusion matrix:
    \[
    ...
    \]
    Additionally, Success Rate (SR) is recorded for each weight triplet \( (w_d, w_c, w_m) \) to measure downstream impact on semantic search success.

    \item \textbf{Evaluation Procedure:}
    \begin{itemize}
        \item Conduct experiments across a range of fusion weight configurations.
        \item Compare performance against:
        \begin{itemize}
            \item detector-only predictions (YOLO-E baseline),
            \item similarity-only filtering (BLIP-2),
            \item memory-only selection,
            \item full hybrid fusion model.
        \end{itemize}
        \item Evaluate per-class results for closed-set, open-vocab, and zero-shot categories.
        \item Analyse how false positives propagate into:
        \begin{itemize}
            \item incorrect graph node generation,
            \item unnecessary navigation actions,
            \item degraded SR and SPL in the overall exploration pipeline.
        \end{itemize}
    \end{itemize}

    \item \textbf{Purpose:}  
    This experiment identifies whether multi-source semantic fusion provides measurable improvements over single-source detection, yielding more robust, stable, and semantically reliable object proposals in challenging indoor environments.

\end{itemize}


\textbf{Experiment 6: Real-World System Performance:}  
\begin{itemize}
