...
The first experiment establishes a performance baseline by comparing the proposed hybrid semantic exploration framework against state-of-the-art methods on standard object-goal navigation metrics. This experiment addresses \textbf{RQ1} by quantifying the overall effectiveness of the integrated exploration–memory approach in realistic indoor environments. Overall performance is evaluated with respect to, using IsaacSIm instead of HabitatSim. \ac{SR} and \ac{SPL} are computed assuming, the captured RGB Image containes the desired object, \ac{SPL} is computed based on the geodesic shortest path from the robot’s initial pose to the nearest instance of the target object. Unlike HabitatSim, sucess is not restricted to tolerance of pre-defined viewpoints, but any position where the object is visible in the RGB image.

For the first experiment, 6 scenes as proposed in Section~\ref{sec:dataset} are used to evaluate the overall performance. Across all scenes, \ac{SR} and \ac{SPL} are computed. \ac{SR} is defined as the proportion of successful episodes in which the robot reaches the queried goal object, defined in Equation~\ref{eq:evaluation:sr},
