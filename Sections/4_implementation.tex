...
Persistent 3D semantic mapping and memory generation are handled by OpenFusion~\cite{yamazakiOpenFusionRealtimeOpenVocabulary2023}, which is wrapped as a dedicated \ac{ROS2} node. OpenFusion incrementally fuses RGB-D observations into a global semantic point cloud representation, serving as the multi-episode persistent memory component of the proposed system.
...
Due to using IsaacSim as the simulation platform instead of HabitatSim~\cite{savvaHabitatPlatformEmbodied2019}, a custom evaluation pipeline is implemented to compute standard object-goal navigation metrics such as \ac{SR} and \ac{SPL}~\cite{puigHabitat30CoHabitat2023}. This ensures comparability with prior work such as VLFM~\cite{yokoyamaVLFMVisionLanguageFrontier2023} and OneMap~\cite{buschOneMapFind2025}, which rely on these widely adopted metrics in embodied \ac{AI} research. The pipeline operates on recorded navigation trajectories and semantic maps generated during each exploration episode and is illustrated in Figure~\ref{fig:evaluation_pipeline}.
...
As described in Chapter~\ref{ch:methods} Section~\ref{sec:methods:semantic_map_construction}, OpenFusion is employed to reconstruct a 3D semantic point cloud from the recorded RGB-D observations~\cite{yamazakiOpenFusionRealtimeOpenVocabulary2023}. The Matterport3D category set is used as the closed set of semantic classes during evaluation~\cite{changMatterport3DLearningRGBD2017}. For each episode, the stored semantic point cloud is filtered according to the target object category specified by the evaluation protocol. All clusters corresponding to the target class are extracted, and their 3D centroids are computed as candidate goal locations. See Figure~\ref{fig:appendix:semantic_evaluation_pointcloud} in Appendix~\ref{ch:appendix:experimental_results} for example semantic pointclouds generated during evaluation.
...
where \( l_i \) is the geodesic shortest path length to the target, \( p_i \) is the executed path length, and \( S_i \) indicates task success for episode \( i \). Following the standard definition of \ac{SR} and \ac{SPL} in embodied navigation benchmarks~\cite{ramakrishnanHabitatMatterport3DDataset2021}, results are aggregated across the $N$ evaluated episodes by the arithmetic mean. Using the mean is consistent with prior work that reports \ac{SR} and \ac{SPL} as mean performance over an evaluation split and therefore enables direct comparison to reported baseline results~\cite{puigHabitat30CoHabitat2023,yokoyamaVLFMVisionLanguageFrontier2023,buschOneMapFind2025}.
...
For Experiments~\ref{sec:experiments:results:2} and~\ref{sec:experiments:results:3}, metrics are aggregated in two stages. Within each multi-object episode (fixed scene, starting pose and hyperparameters), \ac{SR} and \ac{SPL} are first computed per target query and then averaged across the $Q$ sequential queries of that episode (see Equation~\ref{eq:multi_object_episode_aggregation}),

\begin{equation}
\overline{\mathrm{SR}}_e = \frac{1}{Q}\sum_{q=1}^{Q}\mathrm{SR}_{e,q}, 
\qquad
\overline{\mathrm{SPL}}_e = \frac{1}{Q}\sum_{q=1}^{Q}\mathrm{SPL}_{e,q}
\label{eq:multi_object_episode_aggregation}
\end{equation}

yielding one episode-level performance value per configuration. This within-episode mean is appropriate because all queries share the same environment state and initial conditions, and it matches the standard averaging implicit in the definitions of \ac{SR} and \ac{SPL} for single-object episodes, as defined in Metrics of Experiments~\ref{sec:implementation:metrics:experiment1}.

Across the set of episode-level values $\{\overline{\mathrm{SR}}_e\}_{e=1}^{N}$ and $\{\overline{\mathrm{SPL}}_e\}_{e=1}^{N}$ for a given hyperparameter setting, we report the median together with the interquartile range $\mathrm{IQR}=q_{0.75}-q_{0.25}$. The median summarizes central tendency, while the \ac{IQR} quantifies dispersion over the central 50\% of episode-level outcomes. This yields a compact summary of performance variation across different scenes and starting conditions.
...
Success for an individual target is defined identically to Experiment~1, and \ac{SPL} is computed using the geodesic shortest path from the robot’s pose at the start of each sub-task to the nearest instance of the corresponding target object.
...
in Experiment~2 (RQ2). This design enables a direct assessment of whether increased reliance on frontier-based exploration can compensate for semantic noise introduced by higher top-k values. The resulting evaluation scale for Experiment~3 is summarized in Table~\ref{tab:rq3:evaluation_scale}.
...
This experiment evaluates the effectiveness of the multi-source semantic fusion strategy proposed in Chapter~\ref{ch:methods} Section~\ref{sec:methods:fusion_strategy}. Specifically, it studies how Noisy-OR fusion affects the precision-recall trade-off and the prevalence of false positives under higher detection thresholds during exploration, thereby addressing RQ4. Following prior work on open-vocabulary semantic perception, detection performance is evaluated using confusion-matrix-based metrics such as precision, recall, and \ac{F1}-score, which are commonly employed to assess semantic retrieval quality in open-vocabulary settings~\cite{schwaigerOTASOpenvocabularyToken2025,wangYOLOERealTimeSeeing2025,redmonYouOnlyLook2016}.
...
A high \ac{F1}-score indicates that the system effectively balances precision and recall, achieving both accurate and sensitive detection of the target object. The \ac{FPR} quantifies the proportion of incorrect detections among all negative cases and is defined in Equation~\ref{eq:evaluation:fpr},
...
The evaluation focuses on monitoring computational resource utilization, including \ac{CPU}, \ac{GPU}, and RAM usage, as well as inference latency and effective \ac{FPS} of the perception and mapping components during live operation. Each component’s computational load is logged to enable fine-grained performance analysis.

In addition, the semantic map voxel resolution in OpenFusion is systematically reduced to analyze the trade-off between \ac{GPU} memory consumption and runtime performance. This analysis is intended to assess the feasibility of deploying the system on resource-constrained robotic platforms.
