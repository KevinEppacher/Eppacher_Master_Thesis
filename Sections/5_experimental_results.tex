...
experiments. Experiment~1 addresses (RQ1) through a benchmark-style comparison on HM3Dv2.
Experiments~2--4 provide ablations that isolate key design choices of the system:
(i) the exploration-memory weighting (RQ2),
(ii) the semantic memory granularity via OpenFusion retrieval depth (RQ3),
and (iii) multi-source semantic fusion for detection robustness (RQ4).
Finally, Experiment~5 evaluates computational footprint and real-world feasibility (RQ5)
...
Experiment~2 addresses RQ2 by analyzing how the weighting between live exploration signals
...
This experiment addresses RQ5 by evaluating the computational efficiency and real-time feasibility of the proposed system in a real-world deployment setting. Figures~\ref{fig:results:rq5:gpu_vram} and~\ref{fig:appendix:rq5:openfusion_max_voxel} (see Appendix~\ref{ch:appendix:experimental_results}) summarize the \ac{GPU} memory consumption of the overall system and an example of a semantic Map for the given total voxel allocation, respectively.

The \textbf{SAGE} framework consists of three primary \ac{GPU}-intensive components:
...
YOLO-E and BLIP-2 exhibit fixed \ac{GPU} memory footprints of approximately 1200\,MB and 2280\,MB of \ac{VRAM}, respectively, and do not vary during runtime. In contrast to VLFM~\cite{yokoyamaVLFMVisionLanguageFrontier2023}, which utilizes four different \ac{VLM} and detectors, which cumulatively require over 16\,GB of \ac{GPU} memory,~\textbf{SAGE} only uses 3480\,MB for the same functionality, making it more suitable for consumer-grade hardware.

In contrast, the \ac{GPU} memory consumption of OpenFusion depends on the number of allocated voxels in the volumetric map. In the evaluated configuration, a total of 191{,}941 voxels are allocated, resulting in a \ac{GPU} memory usage of approximately 6122\,MB for the semantic memory. This configuration is sufficient to represent an apartment-scale environment, as illustrated in Figure~\ref{fig:appendix:rq5:openfusion_max_voxel}. Overall, the complete \textbf{SAGE} system requires approximately 9.6\,GB of \ac{GPU} \ac{VRAM}, making it compatible with contemporary consumer-grade GPUs.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{Images/05_experimental_results/RQ5/gpu_vram.png}
    \caption{\ac{GPU} memory consumption of the \textbf{SAGE} system during runtime, decomposed by major components.}
