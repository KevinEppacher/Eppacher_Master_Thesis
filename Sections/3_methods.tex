...
\Function{ExtractFrontiers}{
    ...
    \ForAll{unvisited frontier cells $(x,y)$}
        ...
        \If{$N_{\min} \le |\mathcal{P}| \le N_{\max}$}
            ...
            \If{no match found}
                \State \algcomment{Newly discovered frontier}
                \State Assign new frontier ID
            \EndIf

            \State \algcomment{Store valid frontier}
            \State Add $(\mathcal{P}, \mathbf{c})$ to $\mathcal{F}_t$
        \EndIf
    \EndFor
    
    \State \algcomment{Set of clustered, tracked frontiers}
...
\Function{GetScoreFromValueMap}{
    ...
    \ForAll{points $q \in \mathcal{C}$}
        \If{$\lVert q - \mathbf{p} \rVert_2 < r$}
            \State \algcomment{Max semantic response}
...
where $\mathbf{x}_r$ denotes the robot position and $\sigma_r$ controls the spatial
decay of relevance with increasing distance. The angular weighting term is defined in Equation~\eqref{eq:value_map:confidence_weight} in Section~\ref{sec:semantic_frontier:value_map_generation}.

The combined relevance weighting is obtained as the product of both components,
resulting in high relevance values for regions that are close to the robot and aligned
with the camera’s optical axis.

Three instances of the relevance map are maintained with different parameterizations
for exploration, detection, and memory graph nodes, respectively. Graph nodes whose
relevance value exceeds a predefined threshold are suppressed, indicating that the
corresponding region has already been sufficiently observed.
...
exploration of low-value regions.

Figure~\ref{fig:relevance_map_example} illustrates the relevance filtering process
over three consecutive timestamps.
