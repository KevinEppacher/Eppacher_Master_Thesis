...
The value map is generated using a pre-trained \ac{VLM}, specifically \ac{BLIP-2}~\cite{liBLIP2BootstrappingLanguageImage2023}, which is queried via a ROS~2 service to compute the semantic similarity between visual observations and a user-defined text prompt. The value map node subscribes to RGB images, robot poses, and the exploration occupancy grid, following prior semantic frontier formulations~\cite{yokoyamaVLFMVisionLanguageFrontier2023}. The overall value map generation pipeline is illustrated in Figure~\ref{fig:methods:value_map_ros2_pipeline}.
...
For each visible cell $(i,j)$, the semantic value update follows

\begin{equation}
V_t(i,j) = V_{t-1}(i,j) + \alpha\, C_{\text{obs}}(i,j)\big(s_t - V_{t-1}(i,j)\big),
\end{equation}

while the confidence map is updated as

\begin{equation}
C_t(i,j) = \max\big(\lambda C_{t-1}(i,j),\; C_{\text{obs}}(i,j)\big).
\end{equation}
...
$\lambda \in [0,1]$ controls the persistence of past observations and prevents oscillation between multiple similar frontiers by gradually reducing the influence of stale semantic evidence when the robot revisits similar viewpoints.”
...
of view.
Only cells that are geometrically visible from the robot's pose and not occluded by obstacles
are considered for update.
For these visible cells, an instantaneous observation confidence map
$C_{\text{obs}}(\mathbf{p}_t, M_{\text{fov}})$ is computed, which models the reliability of the
current observation as a function of the sensor geometry.
Cells near the center of the field of view are assigned higher confidence, while confidence
decreases toward the periphery due to reduced resolution and increased distortion.
This behavior is modeled using a Gaussian weighting function over the angular deviation from
the camera’s principal viewing direction.
The observation confidence assigned to a visible cell $(i,j)$ is computed as
...
confidence decay within the field of view.
Smaller values of $\sigma$ result in a narrower high-confidence region centered around the
optical axis, while larger values produce a more uniform confidence distribution.

(c) For each visible cell $(i,j)$, the previous semantic value $V_{t-1}(i,j)$ is updated toward the
current similarity score $s_t$, scaled by the effective confidence contribution.
The update gain $\alpha \in [0,1]$ controls how strongly new observations influence existing
values.
At the same time, the confidence map retains the maximum between the decayed previous confidence
and the newly observed confidence, ensuring that strongly observed regions remain influential
over time. Using a max operation ensures that strongly observed regions retain high confidence even after decay, preventing repeated low-confidence observations from diluting reliable semantic evidence. This update mechanism allows the value map to accumulate semantically meaningful information in a spatially consistent manner, while remaining robust to sensor noise, viewpoint changes, and repeated observations from similar poses. 
