...
where $s_i^{(k)}$ denotes the confidence score from source $k$ and $\lambda_k \in [0,1]$ represents the corresponding source weight. The weights $\lambda_k$ allow tuning the influence of each source on the final fused confidence score. Equation~\eqref{eq:graph_fusion:noisy_or_fusion} shows the explicit form of the weighted Noisy-OR fusion for the three considered sources: detection confidence, value map score, and memory relevance.
...
where $\mathcal{N}_{\mathrm{memory}}$ denotes the set of memory nodes within a radius $r_m$ of the detection node location $\mathbf{x}_i$, $s_k$ and $\mathbf{x}_k$ are the confidence score and spatial position of memory node $m_k$, respectively, and $\sigma_m$ controls the spatial decay of the Gaussian kernel. The final graph node representation after multi-source detection fusion is given in Equation~\eqref{eq:graph_fusion:final_node_representation}:
