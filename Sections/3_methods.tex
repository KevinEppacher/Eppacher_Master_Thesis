...
A behavior tree (BT) is implemented to manage the high-level task structure for semantic-guided exploration. The BT orchestrates the robot's actions based on the fused graph nodes produced by the fusion strategy (Section~\ref{sec:methods:fusion_strategy}). It integrates initialization, detection, exploration, and termination behaviors into a cohesive framework.

\subsection*{High-Level Task Structure}

The behavior tree initializes, by publishing the user-defined object prompt and clearing previous maps. It then enters a detection branch, where it continuously monitors for detected objects using the fused detection graph nodes. If an object is detected with confidence exceeding a predefined threshold for a predefined duration (1 second), the robot navigates to the object's location, realigns its pose for optimal viewing, and after verification, captures an image of the object.

If no object is detected, the BT transitions to the exploration branch, where it observes all available exploration-memory graph nodes, which are unexplored frontiers and persistent memory nodes, by spinning in place to gather additional observations. After observation, the robot navigates towards the highest-valued exploration-memory node, as determined by the fusion strategy. This process repeats until either an object is found or a time limit is reached in a closed loop manner. This behavior is illustrated in Figure~\ref{fig:bt:flowchart} as flowchart and as Behavior Tree in Figure~\ref{fig:appendix:bt:tree} in the Appendix.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{Images/03_methods/bt/sage_bt_flowchart.png}
...
The multiple object search behaviors defined in the BT is wrapped in a high-level action server, which allows the user or agent to initiate a search task with a specified object prompt (optional zero-shot object prompt for ValueMap generation), a time limit, a soft and hard confidence threshold for object verification, and other parameters. The behavior tree interacts with the underlying ROS2 navigation stack to execute pre-determined behaviors. The \texttt{GoToGraphNode} uses the \texttt{/navigate\_to\_pose} action interface to send navigation goals to the navigation stack, while the \texttt{ObserveGraphNodes} and \texttt{RealignToObject} hehavior nodes use the \texttt{/spin} action interface to perform in-place rotations for environmental observation and object realignment, respectively. The navigation stack handles low-level path planning, obstacle avoidance, and robot control, allowing the BT to focus on high-level decision-making and task sequencing. The ApproachPoseAdjustor Decorator Node computes if possible, a target approachable pose for the robot to navigate to, given the target graph node position and the current robot pose and the global costmap. If no approachable pose is found, it still uses the original target graph node position as the navigation goal.
