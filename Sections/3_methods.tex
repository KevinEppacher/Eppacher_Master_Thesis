...
In order to search for objects in a zero-shot manner based on user-defined text prompts, a
promptable open-vocabulary object detection model is integrated into the exploration system. Such models leverage large-scale vision-language pre-training to detect and localize objects specified by arbitrary text queries~\cite{radfordLearningTransferableVisual2021, kirillovSegmentAnything2023,wangYOLOERealTimeSeeing2025,liGroundedLanguageImagePretraining2022}.

Table~\ref{tab:methods:object_detection:comparison} compares several state-of-the-art open-vocabulary object detection methods with respect to their prompt types, output representations, foundation models, and real-time capabilities.
...
\begin{table}[H]
    ...
    \bottomrule
\end{tabular}
\caption{Comparison of open-vocabulary object detection methods with respect to supported prompt types and output representations. Assuming 10 fps as real-time capability for low dynamic robotic applications.}
\label{tab:methods:object_detection:comparison}
\end{table}

To generate 3D object hypotheses for the fusion strategy, 2D detections from the selected open-vocabulary object detection model are projected into 3D space using depth information from the RGB-D sensor and camera intrinsics. A common method for extracting 3D locations from 2D detections involves segmenting the detected objects in the image plane, projecting the segmented pixels into 3D using depth values, and clustering the resulting 3D points to obtain robust object locations~\cite{meierCARLADroneMonocular2024,kuJoint3DProposal2018}. Therefore, the chosen detection model must provide instance segmentation masks, confidence scores and have zero-shot capability. 

Detection can be splittet up into three categories: closed-vocabulary, open-vocabulary, and promptable zero-shot detection. Closed-vocabulary models are limited to a fixed set of classes seen during training and cannot generalize to unseen categories~\cite{redmonYouOnlyLook2016,heMaskRCNN2018}. Open-vocabulary models can detect arbitrary object categories specified by text prompts, enabling zero-shot generalization to unseen classes~\cite{liuGroundingDINOMarrying2024,liGroundedLanguageImagePretraining2022,mindererScalingOpenVocabularyObject2024}. Promptable zero-shot detection models further enhance flexibility by allowing both text and image prompts to specify target objects, enabling context-aware detection based on visual cues~\cite{wangYOLOERealTimeSeeing2025,zouSegmentEverythingEverywhere2023}.

SAM~\cite{kirillovSegmentAnything2023} and SEEM~\cite{zouSegmentEverythingEverywhere2023} are promptable zero-shot segmentation models that can generate instance masks based on visual or textual prompts. VLFM~\cite{yokoyamaVLFMVisionLanguageFrontier2023} integrates YOLOv7~\cite{redmonYouOnlyLook2016} for real-time closed-vocabulary detection, grounding DINO~\cite{liuGroundingDINOMarrying2024} for open-vocabulary detection, and SAM~\cite{kirillovSegmentAnything2023} for segmentation. However, VLFM relies on multiple models with varying inference speeds, leading to increased computational overhead and latency. In contrast, YOLO-E~\cite{wangYOLOERealTimeSeeing2025} combines promptable zero-shot detection and instance segmentation into a single efficient model, making it well-suited for real-time robotic applications.

YOLO-E~\cite{wangYOLOERealTimeSeeing2025} conducted a comprehensive evaluationon the LVIS dataset~\cite{guptaLVISDatasetLarge2019} of various open-vocabulary detection models, demonstrating superior performance in terms of accuracy and inference speed. Its architecture builds upon the YOLO framework~\cite{redmonYouOnlyLook2016}, integrating CLIP~\cite{radfordLearningTransferableVisual2021} for vision-language alignment. YOLO-E achieves real-time performance while maintaining high detection accuracy across a wide range of object categories specified by text or image prompts. YOLO-E model \texttt{YOLOE-11-L} uses 26M parameters for textual prompt types and 32M parameters for image prompt types. Compared to GLIPv2~\cite{liGroundedLanguageImagePretraining2022} and Grounding DINO~\cite{liuGroundingDINOMarrying2024} which have 232M and 172M parameters respectively with much less training time, YOLO-E has a \ac{AP} of 35.2/33.7 (text/image prompt) on LVIS while Grounding DINO has an \ac{AP} of 27.4  and GLIPv2 has an \ac{AP} of 29. YOLO-E achieves an inference speed of 130 fps on a T4 GPU and 39.2 fps on a iPhone, making it suitable for real-time applications~\cite{wangYOLOERealTimeSeeing2025}.

This is achieved through a combination of efficient backbone architectures, optimized detection heads, and streamlined processing pipelines that minimize computational overhead without sacrificing accuracy. YOLO-E's ability to handle open-vocabulary object detection based on text prompts, along with its integration of both visual and textual information for robust detection, makes it a compelling choice for real-time robotic applications requiring flexible object recognition capabilities~\cite{wangYOLOERealTimeSeeing2025}.
