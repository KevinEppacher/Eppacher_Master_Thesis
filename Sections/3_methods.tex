...
Robot observations at time $t$ are represented as RGB-D measurements $O_t = \{I_t, D_t\}$, where
...
(a) The \textit{Memory Module} takes as input $O_t$ and $P_t$ and updates the persistent semantic 3D map
$M_t^{\text{sem}}$.
(b) The \textit{Exploration Module} uses $O_t$ and $P_t$ to generate frontier candidates $F_t=\{f_i\}$ from the current geometric exploration map and assigns each candidate a semantic relevance score $s_i^{\text{exp}}$ based on vision-language similarity computed with \ac{BLIP-2}~\cite{liBLIP2BootstrappingLanguageImage2023}.
...
After relevance filtering, the remaining graph nodes are fused in two stages, as illustrated in Figure~\ref{fig:fusion:pipeline}. First, exploration frontier nodes and memory nodes are combined using an exploration-memory weighting scheme (Section~\ref{sec:methods:fusion_strategy:exploration_memory_weighting}), which explicitly trades off short-term exploration incentives against long-term semantic beliefs. Second, detection graph nodes are fused with semantic evidence from the value map and persistent memory via a multi-source detection fusion mechanism (Section~\ref{sec:methods:fusion_strategy:multi_source_detection_fusion}), yielding
...
where $\mathcal{N}_{\mathrm{memory}}$ denotes the set of memory graph nodes, $\mathbf{x}_k$ and $s_k$ are the spatial position and confidence score of memory node $m_k$, and $\sigma_m$ controls the spatial decay of influence. The Gaussian kernel ensures that nearby memory evidence contributes more strongly than distant or outdated observations, while maintaining smoothness and robustness. The memory relevance term acts as a soft spatial prior and does not perform explicit belief updating.
...
Instead of relying solely on the raw detection confidence from a single vision model~\cite{yokoyamaVLFMVisionLanguageFrontier2023}, or solely relying on thresholding the cosine similarity or memory~\cite{gadreCoWsPastureBaselines2022}, the proposed multi-source detection fusion approach combines complementary evidence from multiple independent sources. This leads to higher confidence in true positive detections while reducing the likelihood of false positives, thereby enhancing the robustness and reliability of the object search system by enabling higher confidence thresholds.
...
To preserve robustness against missed detections, graph nodes whose detection
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.4\textwidth]{Images/03_methods/bt/sage_bt_flowchart.png}
...
To increase robustness in cluttered environments, an \texttt{ApproachPoseAdjustor} decorator node computes an alternative, costmap-feasible approach pose based on the target graph node location, the robot’s current pose, and the global costmap. If no valid approach pose can be found, the original graph node position is used as the navigation goal.
