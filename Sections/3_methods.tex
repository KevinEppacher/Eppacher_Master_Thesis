...
A Graph Node $n_i = (\mathbf{x}_i, s_i)$ is defined as a tuple consisting of a 3D position $\mathbf{x}_i \in \mathbb{R}^3$ and an associated semantic score $s_i \in [0,1]$. However, for the purpose of exploration-memory weighting, only the 2D position $\mathbf{x}_i = (x_i, y_i) \in \mathbb{R}^2$ is considered, as the robot operates on a planar navigation surface. This simplification is justified since navigation planning is performed on a 2D costmap, 
and height variations are handled implicitly by the mapping and localization system.
.Similar to classical frontier based approaches~\cite{yamauchiFrontierbasedApproachAutonomous1997,topiwalaFrontierBasedExploration2018}, closer frontiers are rewarded more highly to encourage efficient exploration. The proximity persistence weight $w_{\mathrm{prox}}(n_i)$ is defined in Equation~\eqref{eq:graph_fusion:proximity_weight} using a Gaussian kernel centered at the robot's current position $\mathbf{x}_r = (x_r, y_r)$. The parameters $\alpha_{\mathrm{prox}}$ and $r_{\mathrm{prox}}$ control the maximum weight increase and the spatial decay rate, respectively. This formulation biases the exploration policy toward nearby frontiers while still preserving the relative ordering induced by the semantic score.


% -------------------------------------------------------------
% Proximity persistence (exploration bias)
% -------------------------------------------------------------
\begin{equation}
...
For navigation stability and success, each frontier exploration graph node gets penalized based on the local navigation costmap values. Similar to~\cite{bourgaultInformationBasedAdaptive2002}, a costmap penalty weight $w_{\mathrm{cost}}(n_i)$ is computed for each graph node using a Gaussian kernel to aggregate local costmap values within a radius $r_{\mathrm{cost}}$ (see Equation~\eqref{eq:graph_fusion:costmap_region}). The aggregated cost $\bar{c}_i$ is normalized to the range $[0, 1]$ (see Equations~\eqref{eq:graph_fusion:costmap_weighted_mean} and~\eqref{eq:graph_fusion:costmap_normalization}) and transformed into a penalty weight (see Equation~\eqref{eq:graph_fusion:costmap_penalty}) using the scaling factor $\lambda_{\mathrm{cost}}$. A minimum weight of $0.3$ is enforced to prevent complete suppression of nodes in high-cost areas.
...
c_j \,
e^{-\frac{\lVert \mathbf{x}_j - \mathbf{x}_i \rVert_2^2}{2\sigma_c^2}}
}{
\sum\limits_{j \in \Omega_i}
e^{-\frac{\lVert \mathbf{x}_j - \mathbf{x}_i \rVert_2^2}{2\sigma_c^2}}
}
...
\begin{equation}
\hat{c}_i =
\mathrm{clip}\!\left(\frac{\bar{c}_i}{100}, 0, 1\right)
\label{eq:graph_fusion:costmap_normalization}
\end{equation}
...
After the weighting functions have been defined, exploration frontier nodes are reweighted according to Equation~\eqref{eq:graph_fusion:exploration_score}, where $\beta \in [0, 1]$ controls the trade-off between exploration and memory exploitation.  A higher value of $\beta$ encourages exploration of new areas, while a lower value prioritizes revisiting known high-value regions. In all experiments, $\beta$ is treated as a global hyperparameter that can be adjusted 
to favor exploration-dominated or exploitation-dominated behavior.
% -------------------------------------------------------------
% Exploration node reweighting
% -------------------------------------------------------------
\begin{equation}
s_i^{\mathrm{explore}}
=
...
Memory nodes are scaled using Equation~\eqref{eq:graph_fusion:memory_scaling} to reduce their influence during exploration.

% -------------------------------------------------------------
% Exploitation (memory) node scaling
% -------------------------------------------------------------
\begin{equation}
s_i^{\mathrm{memory}}
=
