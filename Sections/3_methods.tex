...
Similar to thhe gradient ascent analogy in Section~\ref{sec:semantic_frontier:value_map_generation}, the fusion strategy defines the reward function that guides the exploration policy toward promising regions in the environment. By integrating information from multiple sources, including short-term semantic frontiers, persistent memory nodes, and promptable detections, the fusion strategy balances exploration and exploitation to optimize object search performance. First, the frontier graph nodes, memory graph nodes and detection graph nodes are filtered by the Relevance Map (see Section~\ref{sec:methods:fusion_strategy:relevance_filtering}) which build continuously a map, where if Graph Nodes are within the relevance map mask, they are filtered out, to eliminate oscillation. Then the filtered Graph Nodes are fused into a unified representation. Figure~\ref{fig:fusion:pipeline} illustrates the overall fusion pipeline, which consists of two main components: (a) Exploration-Memory Weighting (see Chapter~\ref{sec:methods:fusion_strategy:exploration_memory_weighting}) and (b) Multi-Source Detection Fusion (see Chapter~\ref{sec:methods:fusion_strategy:multi_source_detection_fusion}). After the fusion process, where the Graph Nodes are fused into two Graph Nodes sets: Exploration-Memory Graph Nodes and Detection Graph Nodes, the final Graph Nodes set is forwarded to the BehaviorTree Navigation policy (see Chapter~\ref{sec:methods:behavior_tree}), which orchestrates low-level path planning and robot control to navigate toward high-reward regions identified by the fusion strategy.
...
n_i = (\mathbf{x}_i, s_i),
\qquad
\mathbf{x}_i = (x_i, y_i) \in \mathbb{R}^2,
\quad
s_i \in [0,1]
\label{eq:graph_fusion:node_definition}
\end{equation}

\begin{equation}
\mathbf{x}_r = (x_r, y_r)
\label{eq:graph_fusion:robot_pose}
\end{equation}
