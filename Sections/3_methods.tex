...
\chapter{Methods}
\label{ch:methods}
This section describes the methodology applied in this thesis with a focus on the proposed hybrid semantic exploration system, adressing the issues extracted from the State of the Art in Chapter~\ref{ch:sota}. Firstly, an overview of the system architecture (Section~\ref{sec:methods:system_overview}) is provided, followed by detailed explanations of each component: semantic frontier exploration (Section~\ref{sec:methods:semantic_frontier}), promptable zero-shot detection (Section~\ref{sec:methods:zero_shot_detection}), persistent semantic 3D mapping (Section~\ref{sec:methods:persistent_mapping}), fusion strategy (Section~\ref{sec:methods:fusion_strategy}), and behavior tree for semantic-guided exploration (Section~\ref{sec:methods:behavior_tree}).
...
\begin{itemize}
    ...
    \item Description of data flow between exploration (frontier evaluation), detection (promptable models), and memory (persistent semantic mapping), as shown in Figure~\ref{fig:system_overview}.
...
\subsection*{Exploration 2D Occupancy Map}
