...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        \textbf{No persistent memory} & \acs{VLFM}~\cite{yokoyamaVLFMVisionLanguageFrontier2023}, CoW~\cite{gadreCoWsPastureBaselines2022}, LGX~\cite{dorbalaCanEmbodiedAgent2024}, ESC~\cite{zhouESCExplorationSoft2023} & No long-term fusion or recall; repeated exploration of known areas. \\ \hline
        \textbf{Offline training required} & ZSON~\cite{majumdarZSONZeroShotObjectGoal2023}, PONI~\cite{ramakrishnanPONIPotentialFunctions2022}, PIRLNav~\cite{ramrakhyaPIRLNavPretrainingImitation2023}, \acs{SemExp}~\cite{chaplotObjectGoalNavigation2020} & Heavy RL/supervised training; poor adaptability to new scenes. \\ \hline
        \textbf{No balance between exploration and memory} & \acs{OneMap}~\cite{buschOneMapFind2025}, RayFronts~\cite{alamaRayFrontsOpenSetSemantic2025}, \acs{VLMaps}~\cite{huangVisualLanguageMaps2023} & Either passive mapping or short-term exploration; inefficient search. \\ \hline
        \textbf{No zero-shot exploration} & \acs{VLFM}~\cite{yokoyamaVLFMVisionLanguageFrontier2023}, CoW~\cite{gadreCoWsPastureBaselines2022}, LGX~\cite{dorbalaCanEmbodiedAgent2024} & Detect novel objects but fail to explore unseen regions strategically. \\ \hline
        \textbf{Premapping needed} & \acs{ConceptGraphs}~\cite{guConceptGraphsOpenVocabulary3D2023}, \acs{VLMaps}~\cite{huangVisualLanguageMaps2023}, \acs{GeFF}~\cite{qiuLearningGeneralizableFeature2024} & Depend on pre-recorded data; not suited for online autonomy. \\ \hline
        \textbf{Limited robustness} & PONI~\cite{ramakrishnanPONIPotentialFunctions2022}, \acs{SemExp}~\cite{chaplotObjectGoalNavigation2020}, PIRLNav~\cite{ramrakhyaPIRLNavPretrainingImitation2023} & Closed-set categories; fragile under real-world variation. \\ \hline
        \textbf{Low real-world applicability} & \acs{ConceptGraphs}~\cite{guConceptGraphsOpenVocabulary3D2023}, \acs{VLMaps}~\cite{huangVisualLanguageMaps2023}, Pigeon~\cite{PIGEONVLMDrivenObject} & High GPU cost or simulation-only evaluation; limited deployability on mobile robots. \\ \hline
    \end{tabularx}
...
\begin{enumerate}
    ...
    \textit{Metrics:} Performance is quantified in terms of task success and path efficiency, measured through \acs{SR}, \acs{SPL}, and \acs{MSR} relative to representative state-of-the-art systems such as \acs{OneMap}~\cite{buschOneMapFind2025}, \acs{VLFM}~\cite{yokoyamaVLFMVisionLanguageFrontier2023}, and Pigeon~\cite{PIGEONVLMDrivenObject}.
    ...
    \textit{Metrics:} The weighting factor between exploration and memory is varied during graph node fusion to assess impacts on \acs{SR} and \acs{SPL}, identifying optimal trade-offs between reactivity and exploitation.
    ...
    \textit{Metrics:} The semantic granularity in the 3D semantic mapper is varied while adjusting exploration weight to evaluate effects on \acs{SR} and \acs{SPL}\@.
    ...
    \textit{Metrics:} Precision, Recall, F1-Score, Confusion Matrix, and \acs{SR} under different fusion weight configurations across \acs{COCO}, open-vocabulary, and zero-shot classes.
    
    \item \textbf{What is the computational footprint and real-world robustness of the hybrid framework?} \\
    \textit{Metrics:} \acs{FPS}, GPU/CPU usage, inference latency, and detection stability under sensor noise during physical deployment on a mobile robot.
\end{enumerate}
