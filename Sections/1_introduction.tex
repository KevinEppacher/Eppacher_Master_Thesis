...
\begin{enumerate}
    \item Can the integration of a persistent 3D semantic map improve multi-object search performance in terms of \ac{SR} and \ac{SPL} compared to existing methods without persistent memory?
    \item How does the use of a single vision-language model for frontier scoring impact the real-time performance and memory efficiency of the exploration system compared to multi-model approaches?
    \item To what extent does the proposed sensor-based fusion strategy enhance robustness against false positives in zero-shot object detection during autonomous exploration?
\end{enumerate}
