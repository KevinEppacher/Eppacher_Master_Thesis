...
\begin{itemize}
    \item \textbf{Service Robotics:} Vacuum robot searching for a fridge in the kitchen using a semantic input prompt.
    \item \textbf{Search and Rescue (SAR):} Deployment of one or multiple robots to locate persons, e.g., with the hint that they may be in the bathroom.
    \item \textbf{Inspection:} Robots autonomously exploring areas not accessible via teleoperation, guided by semantic input.
    \item \textbf{Warehouse Automation:} Large warehouses requiring robots to locate items, pallets, or storage units that may not be consistently labeled or may be partially occluded.
\end{itemize}
