...
\begin{itemize}
    \item \textbf{Service Robotics:} A domestic vacuum robot may be instructed to find and approach a specific household object, such as a fridge in the kitchen, using a semantic input prompt rather than fixed coordinates or predefined waypoints.
    \item \textbf{Search and Rescue (SAR):} In emergency scenarios, autonomous ground or aerial robots are deployed to search for persons with incomplete or uncertain information, such as a hint that a missing individual may be located in a bathroom or behind a closed door.
    \item \textbf{Inspection:} Industrial or infrastructure inspection robots must autonomously explore and analyze previously uncharted or unteleoperable areas, guided by semantic instructions (e.g., “inspect the corroded pipe near the valve”).
    \item \textbf{Warehouse Automation:} Large-scale warehouses often contain thousands of storage units or pallets that may not be consistently labeled or easily visible. Semantic perception enables robots to efficiently locate and retrieve items that match high-level descriptions (e.g., “find the blue container behind the conveyor belt”).
\end{itemize}
