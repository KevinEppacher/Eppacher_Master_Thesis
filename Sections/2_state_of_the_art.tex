...
Therefore, recent research has shifted toward leveraging large-scale pretrained vision-language models that provide rich semantic representations and zero-shot generalization capabilities. By integrating these models into modular exploration frameworks, it is possible to overcome many of the limitations of learned policies while retaining the benefits of semantic reasoning for efficient multi-object search.

\cite{zhouESCExplorationSoft2023} proposed \ac{ESC}, an exploration framework that integrates semantic cues from \ac{VLM}`s into frontier-based exploration. The approach combines a pretrained \ac{GLIP}~\cite{liGroundedLanguageImagePretraining2022} object detector with a \ac{LLM} (ChatGPT or DeBERTa) to generate semantic priors and guide exploration decisions. It scores frontiers based on both geometric utility and semantic relevance to the target object, enabling more efficient search behavior, shown in Equation~\ref{eq:esc}:
...
where \(P(F)\) is the probability of selecting frontier \(F\), conditioned on detected objects \(d_i^{t}\), current observations \(o^{t}\), and the robot's pose \(r^{t}\) at time \(t\). This called Probabilistic Soft Logic (PSL) scores frontiers by combining visual detections with language-derived priors about object co-occurrences and spatial relationships. The robot then selects frontiers with the highest semantic relevance for exploration. \cite{zhouESCExplorationSoft2023} uses \ac{GLIP} as a detection backbone, which computes 2D bounding boxes and class labels for objects in the robot's \ac{FOV} and a confidence score for each detection. Detection is also confidence-threshold dependent. Disadvantages of \ac{ESC} include the computational overhead from LLM inference and PSL optimization. It also relies heavily on the quality of object detections; false positives can mislead frontier scoring and exploration decisions. Also the commonsense prior can be incorrect or misleading. \cite{zhouESCExplorationSoft2023} mentions, due to ablations studies, that the object-object and object-room relation priors are misleading and that commonsense relations are probabilistic rather than deterministic. \ac{ESC} maintains a semantic map during navigation but does not explicitly model long-term semantic memory or belief accumulation across revisits. Once incorrect detections or priors are introduced, there is no learned mechanism to revise or down-weight them over time, as acknowledged indirectly in the discussion of detection-driven failures.
