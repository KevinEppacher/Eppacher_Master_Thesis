...
\citeauthor{ramrakhyaPIRLNavPretrainingImitation2023}~\cite{ramrakhyaPIRLNavPretrainingImitation2023} introduced PIRLNav, a pretraining framework for vision-language navigation that combines behavioral cloning and reinforcement learning to learn generalizable exploration policies. The approach consists of two main stages. In the first stage, a policy network is pretrained using behavioral cloning on a large dataset of expert demonstrations. The objective is to minimize the negative log-likelihood of the expert actions given the observations, which can be expressed in Equation~\ref{eq:pirlnav_bc}.
...
The observations \(o_t\) include RGB images, GPS + Compass HM3D Pose, One-hot encoded target object category, and an implicit memory via recurrent network (GRU). The action space consists of discrete navigation commands: \textit{move forward}, \textit{turn left}, \textit{turn right}, and \textit{stop}. $\theta^{*}$ is the policy network parameterized by $\theta$ and $(o_t, a_t)$ are observation-action pairs from expert demonstrations.
 This stage teaches human like navigation behavior. Output is a pretrained navigation policy which maps observations to actions with human like behavior.

The true contribution of PIRLNav~\cite{ramrakhyaPIRLNavPretrainingImitation2023} comes in stage two, where the pretrained policy is fine-tuned using \ac{DRL} to optimize long-term exploration objectives. The actor policy is initialited with the weights from stage one and the value function (critic) network is trained first, while keeping the actor frozen. Once the critic is stable, both networks are trained jointly using Proximal Policy Optimization (PPO). The reinforcement learning objective is to maximize the expected cumulative discounted reward over trajectories sampled from the policy, as shown in Equation~\ref{eq:pirlnav_drl}.
...
in which \(\tau\) represents trajectories of observations, actions, and rewards generated by following policy \(\pi\), \(r_t\) is the reward at time step \(t\), and \(\gamma\) is the discount factor. The reward function returns 1, if agent stoppes within a specific range and is in the \ac{FOV}. In PIRLNav~\cite{ramrakhyaPIRLNavPretrainingImitation2023}, there is no explicit detector used to determine if the target object is in the \ac{FOV}, instead the detector is embedded implicitly in the learned policy through the pretraining stage, with pre-trained \ac{DINO}~\cite{jiangCLIPDINOVisual2024} visual features. 

Limited interpretability due to end-to-end learning, No explicit semantic memory, retraining needed if environment changes significantly in visual style and sensor characteristics, Generalization depends on similarity between training and test environments.
