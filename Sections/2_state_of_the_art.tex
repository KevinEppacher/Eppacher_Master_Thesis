...
Unlike ~\cite{chenHowNotTrain2023}, which converts classes of detection into a value map, and possibly loosing information or adding misinformation,\ac{VLFM}~\cite{yokoyamaVLFMVisionLanguageFrontier2023} directly uses the image-text similarity to compute the from the entire image. The cosine similarity \ref{eq:cosine_similarity}~\cite{gadreCoWsPastureBaselines2022} is computed with a pre-trained VLM from BLIP-2~\cite{liBLIP2BootstrappingLanguageImage2023} between the current observation and the target text query. The similarity values are then projected onto a 2D occupancy grid in a \ac{FOV} mask, where towards the border of the image \ac{FOV} mask values are lower due to lower confidence of the detection (gaussian weighting). The resulting value map is then masked with a geometric frontier map to select the next exploration goal. \ac{VLFM}~\cite{yokoyamaVLFMVisionLanguageFrontier2023} uses GroundingDINO~\cite{liuGroundingDINOMarrying2024}, for zero-shot detection or YOLOv7 for COCO \cite{linMicrosoftCOCOCommon2015} trained classes, as object detectors to detect objects in the current observation, which are then segmented with SAM~\cite{kirillovSegmentAnything2023} to get precise object masks. The detected objects are then used to determine if the target object is in the current observation and stop the exploration and navigate towards the object. The results are promising, however, \ac{VLFM}~\cite{yokoyamaVLFMVisionLanguageFrontier2023} also suffers from false positives in the zero-shot detection setting, which can lead to wrong stopping decisions, due to only either having a single source of detection pipeline (GroundingDINO~\cite{liuGroundingDINOMarrying2024} or YOLOv7). The value map is episodic and not persistent over time, which can lead to revisiting already explored areas, which is inefficient for multi-object search. \ac{VLFM} has real-time properties, but due to using 4 different models (BLIP-2~\cite{liBLIP2BootstrappingLanguageImage2023}, GroundingDINO~\cite{liuGroundingDINOMarrying2024}/YOLOv7, SAM~\cite{kirillovSegmentAnything2023}), it uses 16 GB VRAM on an NVIDIA RTX 4090 GPU, which is quite high for real-time applications on mobile robots.
